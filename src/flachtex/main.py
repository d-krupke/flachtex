...
def parse_arguments():
    ...
    parser.add_argument(
        "--format",
        action="store_true",
        help="Format output for diff-friendly version control (sentence-per-line).",
    )
    parser.add_argument(
        "--indent",
        type=int,
...
def main():
    ...
    if args.format or args.indent > 0:
        doc = format_latex(doc, indent=args.indent, sentence_per_line=args.format)
